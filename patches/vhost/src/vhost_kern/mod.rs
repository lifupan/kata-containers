...
use vm_memory::{Address, GuestAddress, GuestAddressSpace, GuestMemory, GuestMemoryBackend, GuestUsize};
...
pub trait VhostKernBackend: AsRawFd {
    ...
    fn is_valid(&self, config_data: &VringConfigData) -> bool {
        ...
        let pm = match m.physical_memory() {
            Some(pm) => pm,
            None => return false,
        };
        ...
        if GuestAddress(config_data.desc_table_addr)
            ...
            .is_none_or(|v| !pm.address_in_range(v))
        {
            return false;
        }
...
impl VringConfigData {
    ...
    pub fn to_vhost_vring_addr<AS: GuestAddressSpace>(
        ...
        let pm = m
            .physical_memory()
            .ok_or(Error::InvalidGuestMemory)?;
        let desc_addr = pm
