...
use kvm_bindings::{kvm_irq_routing_entry, KvmIrqRouting as KvmIrqRoutingTable};
use kvm_ioctls::VmFd;
...
impl KvmIrqRouting {
    ...
    fn set_routing(&self, routes: &HashMap<u64, kvm_irq_routing_entry>) -> Result<()> {
        ...
        let irq_routing =
            KvmIrqRoutingTable::from_entries(&entries).map_err(|_| {
                Error::new(ErrorKind::Other, "failed to create KvmIrqRouting")
            })?;
