...
use kvm_bindings::{kvm_irq_routing_entry, KvmIrqRouting as KvmIrqRoutingFam};
use kvm_ioctls::VmFd;
...
impl KvmIrqRouting {
    ...
    fn set_routing(&self, routes: &HashMap<u64, kvm_irq_routing_entry>) -> Result<()> {
        let entries: Vec<kvm_irq_routing_entry> = routes.values().copied().collect();
        let irq_routing = KvmIrqRoutingFam::from_entries(&entries)
            .map_err(|_| std::io::Error::new(ErrorKind::Other, "failed to create irq routing"))?;
