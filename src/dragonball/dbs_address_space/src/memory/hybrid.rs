...
use vm_memory::{
    ...
    GuestRegionMmap, GuestUsize, MemoryRegionAddress, ReadVolatile, VolatileSlice, WriteVolatile,
};

use crate::GuestRegionRaw;

/// Errors for `GuestMemoryHybrid` operations.
...
pub enum Error {
    ...
    #[error("Invalid guest region")]
    InvalidGuestRegion,
}

impl From<GuestRegionCollectionError> for Error {
    fn from(e: GuestRegionCollectionError) -> Self {
        match e {
            GuestRegionCollectionError::NoMemoryRegion => Error::NoMemoryRegion,
            GuestRegionCollectionError::MemoryRegionOverlap => Error::MemoryRegionOverlap,
            GuestRegionCollectionError::UnsortedMemoryRegions => Error::UnsortedMemoryRegions,
        }
    }
}
