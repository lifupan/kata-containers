        ...
        for io in data_descs {
            match self.request_type {
                RequestType::In => {
                    ...
                    disk.read_exact(&mut buf).map_err(|e| {
                        ExecuteError::Read(GuestMemoryError::IOError(e))
                    })?;
                ...
                RequestType::Out => {
                    ...
                    disk.write_all(&buf).map_err(|e| {
                        ExecuteError::Write(GuestMemoryError::IOError(e))
                    })?;
                }
