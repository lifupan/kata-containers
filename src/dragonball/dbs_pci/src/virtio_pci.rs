...
use crate::{
    MsixCap, MsixState, PciBarConfiguration, PciBarRegionType, PciBus, PciCapability,
    PciCapabilityId, PciClassCode, PciConfiguration, PciDevice, PciHeaderType,
    PciMassStorageSubclass, PciNetworkControllerSubclass, PciSubclass, PciBarPrefetchable,
};

use super::pci_common_config::{
    VirtioPciCommonConfig, VirtioPciCommonConfigState,
};
use dbs_virtio_devices::{TYPE_BLOCK, TYPE_NET};

use dbs_interrupt::{VirtioNotifierMsix, VirtioInterruptType, InterruptNotifier, VIRTQ_MSI_NO_VECTOR};

use dbs_virtio_devices::{VirtioDevice, VirtioDeviceConfig, VirtioQueueConfig, ActivateError, 
    ActivateResult, VirtioSharedMemoryList, DEVICE_ACKNOWLEDGE, DEVICE_DRIVER, DEVICE_DRIVER_OK, DEVICE_FAILED,
    DEVICE_FEATURES_OK, DEVICE_INIT};


//use crate::transport::dragonball::{DeviceFeatures, VIRTIO_PCI_VENDOR_ID_DRAGONBALL};
...
pub struct VirtioPciDevice<AS: GuestAddressSpace + Clone + 'static, 
    Q: QueueT,
    R: GuestMemoryRegion> {
    dev_id: u8,
    vm_fd: Arc<VmFd>,
    ...
    device: Arc<Mutex<Box<dyn VirtioDevice<AS, Q, R>>>>,
    ...
    R: GuestMemoryRegion,
{
    ...
    pub fn get_interrupt_requirements(
        ...
        requests.push(ResourceConstraint::PciMsixIrq {
            size: (device.queue_max_sizes().len() + has_ctrl_queue + 1)
                as u32,
        });
    }
    ...
    pub(crate) fn create_queues(
        ...
        for (idx, size) in device.queue_max_sizes().iter().enumerate() {
            queues.push(VirtioQueueConfig::create(*size, idx as u16).map_err(VirtioPciDeviceError::VirtioQueue)?);
        }
        ...
        if has_ctrl_queue {
            queues.push(VirtioQueueConfig::create(device.ctrl_queue_max_sizes(), queues.len() as u16).map_err(VirtioPciDeviceError::VirtioQueue)?);
    ...
    pub fn arc_device(&self) -> Arc<Mutex<Box<dyn VirtioDevice<AS, Q, R>>>> {
        self.device.clone()
    }
...
impl<AS: GuestAddressSpace + Clone + 'static,
    Q: QueueT + Clone + 'static,
    R: GuestMemoryRegion + 'static,> VirtioPciDevice<AS, Q, R> {
    ...
    fn activate(&self) -> ActivateResult {
        ...
        let interrupt_source_group = self.intr_mgr().get_group().ok_or(
            ActivateError::VirtioPci,
        )?;
    ...
    /// TODO(zixi): used by pci bar reprograming
    #[allow(dead_code)]
    pub fn move_bar(
        &mut self,
        old_base: u64,
...
impl<AS: GuestAddressSpace + Clone + 'static + Send + Sync, 
...
pub(crate) mod tests {
    ...
    use vm_device::resources::MsiIrqType;
    ...
    use crate::{
        ActivateResult, Result as VirtIoResult, VirtioDeviceConfig, VirtioDeviceInfo,
        VirtioSharedMemory,
    };
    ...
    impl VirtioDevice<Arc<GuestMemoryMmap>> for DummyDevice {
        fn device_type(&self) -> VirtioDeviceType {
            VirtioDeviceType::Dummy
        }
        ...
        ) -> VirtIoResult<Option<VirtioSharedMemoryList>> {
    ...
    fn get_pci_device() -> VirtioPciDevice<Arc<GuestMemoryMmap>> {
        ...
        let mut pci_dev = VirtioPciDevice::new(
            ...
            Some(DeviceFeatures::AUX_NOTIFICATION_BRIDGE | DeviceFeatures::PVDMA),
            setting_bar_res,
        )
        .unwrap();

        pci_dev.alloc_bars().unwrap();
        assert!(pci_dev.enable_aux_bridge);
        let mut data = [0u8; 4];
        pci_dev
            .configuration
            .lock()
        ...
        let vendor_id = LittleEndian::read_u32(&data);
        assert!(vendor_id & DeviceFeatures::PVDMA.bits() != 0);
        // has 2 aux notifiction
        assert_eq!(pci_dev.state.lock().unwrap().aux_infos.len(), 2);
        assert_eq!(
            pci_dev
                .aux_msi_config
                .lock()
    ...
    #[cfg(target_arch = "x86_64")]
    fn activate_device(d: &mut VirtioPciDevice<Arc<GuestMemoryMmap>>) {
    ...
    #[cfg(target_arch = "x86_64")]
    fn set_driver_status(d: &mut VirtioPciDevice<Arc<GuestMemoryMmap>>, status: u8) {
        d.write(
            IoAddress(0),
    ...
    #[cfg(target_arch = "x86_64")]
    fn get_driver_status(d: &mut VirtioPciDevice<Arc<GuestMemoryMmap>>) -> u8 {
    ...
    fn test_get_interrupt_requirements() {
        ...
        assert!(matches!(req[0], ResourceConstraint::PciMsixIrq{ size } if size == 2 + 2 + 1));
    }
    ...
    fn test_read_bar() {
        ...
        d.arc_device().lock().unwrap().read_config(0, &mut new_data);
        assert_eq!(origin_data, new_data);
    }
}
