...
use dbs_virtio_devices::{VirtioDevice, VirtioQueueConfig};
...
impl VirtioPciCommonConfig {
    ...
    // TODO(tianqian.zyf): use for live upgrade later
    #[allow(dead_code)]
    ...
    fn read_common_config_word<Q: QueueT + 'static>(&self, offset: u64, queues: &[VirtioQueueConfig::<Q>]) -> u16 {
    ...
    fn read_common_config_dword<AS: GuestAddressSpace + 'static, Q: QueueT + 'static,
    ...
    fn write_common_config_dword<AS: GuestAddressSpace + 'static, Q: QueueT + 'static,
     R: 'static + GuestMemoryRegion>(
        &mut self,
        offset: u64,
        value: u32,
    ...
    fn with_queue_mut<F: FnOnce(&mut Q), Q: QueueT + 'static>(&self, queues: &mut [VirtioQueueConfig::<Q>], f: F) {
        if let Some(queue) = queues.get_mut(self.queue_select as usize) {
            f(&mut queue.queue);
        }
    }

    pub fn read<AS: GuestAddressSpace + 'static, Q: QueueT + 'static,
     R: 'static + GuestMemoryRegion>(
        &self,
        offset: u64,
        data: &mut [u8],
    ...
    pub fn write<AS: GuestAddressSpace + 'static, Q: QueueT + 'static,
     R: 'static + GuestMemoryRegion>(
        &mut self,
        offset: u64,
        data: &[u8],
...
mod tests {
    ...
    use super::super::pci_device::tests::{DummyDevice, DUMMY_FEATURES};
    use super::*;
    use crate::VirtioDevice;

    #[test]
    fn test_aux_notification() {
        let mut aux_notification = VirtioPciDriverAuxNotificationConfig::new(2);
        let mut data = [0u8; 2];
        aux_notification.read(0, &mut data);
        let value = LittleEndian::read_u16(&data);
        assert_eq!(value, 0);

        LittleEndian::write_u16(&mut data, 1);
        aux_notification.write(0, &data);
        assert_eq!(aux_notification.select, 1);
        aux_notification.read(0, &mut data);
        let value = LittleEndian::read_u16(&data);
        assert_eq!(value, 1);

        LittleEndian::write_u16(&mut data, 0);
        aux_notification.write(0, &data);
        assert_eq!(aux_notification.select, 0);
        LittleEndian::write_u16(&mut data, 0xf);
        aux_notification.write(2, &data);
        assert_eq!(aux_notification.msix_vectors.lock().unwrap()[0], 0xf);

        // invalid offset
        let mut data = [0u8; 2];
        aux_notification.write(3, &data);
        assert_eq!(aux_notification.msix_vectors.lock().unwrap()[0], 0xf);

        aux_notification.read(2, &mut data);
        let value = LittleEndian::read_u16(&data);
        assert_eq!(value, 0xf);

        let mut data = [0u8; 2];
        // invalid offset
        aux_notification.read(3, &mut data);
        let value = LittleEndian::read_u16(&data);
        assert_eq!(value, 0);
    }
    ...
    fn write_base_regs() {
        ...
        let dev: Arc<Mutex<Box<dyn VirtioDevice<Arc<GuestMemoryMmap>>>>> =
